...
from escrow.get_escrow_wallet import get_escrow_wallet_detail, get_escrow_wallet_from_request
...
async def test_get_escrow_wallet_from_request(mock_get_escrow_wallet):
    ...
    req = make_mocked_request(
        'GET', reverse('escrow-address', escrow_address=wallet_address), match_info={'wallet_address': wallet_address}
    )
...
async def test_get_escrow_wallet_success_trusted_htkn(mock_address):
    ...
    expect_data = {
        ...
        'data': {'name': 'UnitTest', 'age': '30'},
        'signers': [{'public_key': 'provider_address', 'weight': 1}, {'public_key': 'creator_address', 'weight': 1}],
    }
