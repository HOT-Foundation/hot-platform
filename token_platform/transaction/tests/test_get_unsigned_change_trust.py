...
class TestGetUnsignedChangeTrust(BaseTestClass):
    ...
    async def test_get_unsigned_change_trust_success(
        self, mock_get_threshold_weight, mock_get_signer, mock_get_stellar_wallet
    ):
        mock_get_threshold_weight.return_value = 1
        mock_get_signer.return_value = [
            {"public_key": "GAGNG7WP6JJH726KJ3RPMHB3TNOVNABRBHULYVN3APK6CHXRJNRSSHBA", "weight": 1}
        ]
...
class TestGetUnsignedAddTrustAddHot(BaseTestClass):
    @unittest_run_loop
    @patch('transaction.get_unsigned_change_trust.get_unsigned_add_trust_and_hot')
    @patch('transaction.get_unsigned_change_trust.does_wallet_have_trust')
    async def test_get_unsigned_add_trust_and_hot_from_request_success(
        self, mock_has_wallet, mock_get_unsigned_add_trust_and_hot
    ):
        mock_get_unsigned_add_trust_and_hot.return_value = {}
        ...
        resp = await self.client.request('GET', url)
        assert resp.status == 200
        mock_get_unsigned_add_trust_and_hot.assert_called_once_with(
            wallet_address, transaction_source_address, Decimal(hot_amount)
        )

    @unittest_run_loop
    @patch('transaction.get_unsigned_change_trust.get_unsigned_add_trust_and_hot')
    @patch('transaction.get_unsigned_change_trust.does_wallet_have_trust')
    async def test_get_unsigned_add_trust_and_hot_from_request_error_not_enough_parameter(
        self, mock_has_wallet, mock_get_unsigned_add_trust_and_hot
    ):
        mock_get_unsigned_add_trust_and_hot.return_value = {}
        mock_has_wallet.return_value = False
    ...
    @unittest_run_loop
    @patch('transaction.get_unsigned_change_trust.Builder')
    async def test_build_unsigned_add_trust_and_hot(self, mock_builder):
        instance = mock_builder.return_value
        instance.append_trust_op.return_value = {}
        ...
        with pytest.raises(web.HTTPNotFound):
            await build_unsigned_add_trust_and_hot(
                'GDHZCRVQP3W3GUSZMC3ECHRG3WVQQZXVDHY5TOQ5AB5JKRSSUUZ6XDUE',
                'GDHZCRVQP3W3GUSZMC3ECHRG3WVQQZXVDHY5TOQ5AB5JKRSSUUZ6XDUE',
                Decimal(5),
            )
