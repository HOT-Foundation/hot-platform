...
from transaction.transaction import submit_transaction
...
class TestSubmitTransactionFromRequest(BaseTestClass):
    ...
    async def test_submit_transaction_from_request_success(self, mock) -> None:
    ...
    @unittest_run_loop
    @patch('transaction.transaction.submit_transaction')
    async def test_submit_transaction_from_request_error_wrong_parameter(self, mock) -> None:
        mock.return_value = {'status': 200}
        url = f'transaction/transaction-hash'
