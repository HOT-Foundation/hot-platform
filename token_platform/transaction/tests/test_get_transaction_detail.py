...
class TestGetTransactionFromRequest(BaseTestClass):
    ...
    @unittest_run_loop
    @patch('transaction.transaction.stellar.wallet.get_operations_of_transaction')
    @patch('transaction.transaction.stellar.wallet.get_transaction')
    async def test_get_transaction_success(self, mock_transaction, mock_operations):
    ...
    async def test_get_transaction_not_found(self, mock_transaction):
        ...
        mock_transaction.side_effect = web.HTTPNotFound(
            reason='The resource at the url requested was not found.  This is usually occurs for one of two reasons:  The url requested is not valid, or no data in our database could be found with the parameters provided.'
        )
    ...
    async def test_get_transaction_hash_by_memo_from_reqeust_can_get_tx(self, mock_get_transaction) -> None:
        ...
        resp = await self.client.request(
            'GET', reverse('get-transaction-hash-memo', wallet_address='address', memo='hello')
        )
        assert resp.status == 200
